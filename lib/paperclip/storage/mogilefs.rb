    ...
    module Mogilefs
      ...
      def flush_writes #:nodoc:
        @queued_for_write.each do |style, file|
          logger.info("[paperclip] saving #{url(style)} to MogileFS")
      ...
      def flush_deletes #:nodoc:
        @queued_for_delete.each do |path|
          logger.info("[paperclip] deleting #{path} from MogileFS")

          begin
            mogilefs.delete(path)
          rescue MogileFS::Backend::UnknownKeyError
            logger.info("[paperclip] Error: #{path} not found in MogileFS")
          end
        end
        @queued_for_delete = []
      end
